...
mkdir $NSNS_DIR
cd $NSNS_DIR
sed -i 's/    "MaxRuntimeHours"   => "[[:digit:]]\+",/    "MaxRuntimeHours"   => "'"$hours"'",/g' /RQusagers/tvincent/SpEC/Support/Perl/Machines.pm
...
sed -i 's/,//g' TOV_output.txt
python /scratch/p/pfeiffer/tvincent/BNS_Disks_project/Scripts/tab_dmgen_2shell.py
