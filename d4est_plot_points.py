...
for p in range(0,num_points):
    ...
    for i in range(0,len(point_rows[2])):
        ...
        if p == 1:
            ...
            low_err = y[-1]
            high_dof = x[-1]
    plt.plot(x,y,label=labels[p],color='black',linestyle=linestyles[p], marker='o', markersize=1)

l = plt.legend(loc=0, prop={'size':5})
...
plt.grid()
plt.tight_layout()
plt.savefig("points_" + str(high_dof) + "_" + str(m) + "_" + str(low_err) + ".pdf",rasterized=False)
